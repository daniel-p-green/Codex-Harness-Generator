...
validation checklist. `.claude/rules/03-quality-gates.md` (the always-loaded
...
- v2 (2026-05-31): designated single source of truth. Added the precise check-
  ...
  vs-files boundary crossing), 20b (GENESIS vocabulary in CLAUDE.md). Added tier
...
**1. CLAUDE.md file references exist**: Read CLAUDE.md, extract every file path
reference (rules, agents, skills, docs). Verify each exists on disk.
FAIL if any referenced file is missing.

**2. Agent frontmatter validity**: Every `.claude/agents/*.md` must have YAML
frontmatter with `name` (matches filename stem), `description` (non-empty),
`tools` (list), `maxTurns` (integer > 0). WARN if `model` missing. FAIL if
required fields missing.

**2b. disallowedTools are bare tool names**: Every agent `disallowedTools` entry
must be a bare tool name (e.g. `Write`, `Edit`, `Bash`) -- never a path-scoped or
glob form like `Write(./Brand/**)`. A glob there is unrecognized and fails open
silently, so a "protected path" guarantee is inert. Path restrictions belong in
settings.json `permissions.deny`. FAIL on any disallowedTools entry containing `(`
or a path/glob character.

**3. Skill frontmatter validity**: Every `.claude/skills/*/SKILL.md` must have
`description` (80-1024 chars, 3+ trigger phrases, third person), `context`
(must be `fork`), `allowed-tools`. Check description quality: states WHAT + WHEN,
includes negative triggers if name is ambiguous. FAIL if < 3 triggers or < 80 chars.

**4. No README.md in skill folders**: Glob for `.claude/skills/*/README.md`.
Any match is FAIL.

**5. settings.json validity**: Must be valid JSON with `permissions.deny` array
containing at least one entry. FAIL if invalid JSON or no deny rules.

**6. settings.json permission coverage**: Cross-reference agent tool lists and
skill allowed-tools against settings.json permissions. Deny rules must cover
destructive commands and sensitive file reads. WARN if an agent uses a tool not
...
CLAUDE.md. Every skill referenced in CLAUDE.md or a rule. Every rule referenced
in CLAUDE.md. WARN if 1 unreferenced, FAIL if 2+.

### Size and Quality Checks (14-17) -- BLOCKING (size limits) / CRITICAL (quality)

**14. File size limits**: CLAUDE.md < 250 lines (FAIL). Rule files < 120 lines
...
`.claude/` component on disk must appear in the manifest. This is a connection
...
**19. .claudeignore present**: Must exist and be non-empty with appropriate
project-type patterns. WARN if missing or minimal.

**20. Intent-behind-rules in CLAUDE.md**: Sample 5+ constraints. 80%+ must
include WHY (because..., parenthetical explanation). WARN if 50-79%. FAIL < 50%.

**20b. GENESIS vocabulary in CLAUDE.md (no jargon drift)**: The domain
vocabulary the user gave in GENESIS.md (key nouns, tools, workflow terms) must
actually appear in the generated CLAUDE.md and routing table. A generated
environment whose CLAUDE.md reads generically -- as if it could belong to any
project -- failed to absorb the intake. Sample the 5-8 most distinctive GENESIS
terms; CRITICAL if fewer than half appear anywhere in CLAUDE.md or the routing
rule.
...
**23. PreCompact hook**: settings.json must configure PreCompact auto-save hook
(or document in GETTING_STARTED.md).

**24. settings.local.json**: Must be generated when ARCHITECTURE.md specifies
...
component, so this effectively always applies. settings.json must configure a
...
settings.json must configure an InstructionsLoaded hook (preferred over SessionStart)
...
**42. Fast mode documented in GETTING_STARTED.md**: When GENESIS.md indicates
cost-conscious or balanced efficiency priority, GETTING_STARTED.md must document
fast mode availability and cost trade-offs. Cost-conscious environments must include
`fastModePerSessionOptIn: true` in settings.json. WARN if missing.

**43. MCP server packages verified**: Every MCP server in generated .mcp.json or
settings.json must correspond to a verified server from tool-registry.md "Verified
...
**44. Hook matcher format**: All hook matchers in settings.json must be strings
...
**47. Opus 4.7 API compatibility**: When GENESIS.md targets Opus 4.7 (or does not
specify a pinned older model), generated settings.json, skill bodies, agent
bodies, and embedded code snippets must NOT set `temperature`, `top_p`, `top_k`,
or `thinking.budget_tokens`. These parameters return 400 on Opus 4.7. FAIL if
any such parameter appears in generated API-call content. PASS if they only
appear inside clearly-marked legacy/4.6-fallback sections.

**48. Effort level guidance**: When the generated environment targets Opus 4.7,
GETTING_STARTED.md must mention `effort` tuning. Must include at least one of:
`effort: xhigh` recommendation for coding/agentic work, the `/effort`
interactive slider, or the `effort:` frontmatter field on agents/skills. WARN
if absent.

**49. Tokenizer headroom**: When the generated environment targets Opus 4.7,
any embedded `max_tokens` values in example API snippets should be at least
as large as 1.3x the comparable 4.6 value, or at least 64k for agentic runs.
WARN if max_tokens appears as 4k or 8k in example agentic-loop snippets.
...
`<target>/<area-slug>/CLAUDE.md`, and `<target>/<area-slug>/.claude/`.
FAIL if any registry entry has no matching subfolder, or if a subfolder
exists with `.claude/` but is not listed in the registry.

**51. Cumulative CLAUDE.md budget**: Measure parent CLAUDE.md line count plus
each per-area CLAUDE.md line count. The largest (parent + any one child) must
be under 250 lines. Parent CLAUDE.md alone should be under 80 lines.
...
direct file paths to sibling areas (e.g., `../other-area/.claude/skills/foo/`).
...
**54. Parent permission subset**: Parent settings.json `permissions.allow` must
be a subset of the union of permissions each area would need. No parent-only
allow rules that no area uses -- those belong in the specific area that needs
them. WARN if parent grants a permission no area uses; delete or move it.
...
shared rule/skill/agent listed must exist at `<target>/.claude/`, and every
`<target>/.claude/` component (except the work-area registry artifacts) must
