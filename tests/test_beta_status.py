...
class BetaStatusTests(unittest.TestCase):
    ...
    def test_build_payload_summarizes_missing_evidence_and_waiting_pilot_queue(self):
        ...
        self.assertEqual("llm-app-pilot", payload["waiting_followups"][0]["slug"])
        self.assertEqual([], payload["conversion_ready"])
    ...
    def test_write_report_includes_dashboard_sections(self):
        ...
        self.assertIn("## Waiting Issues", text)
        self.assertIn("### llm-app-pilot", text)
        self.assertIn("Missing fields: outcome, task_summary, evidence, verification, privacy_review, limitations", text)
        self.assertIn("## Conversion Ready Issues", text)
