...
- 10.11 MCP Tool Search for Context Reduction [Codex]
- 10.12 Browser Automation Tools [ALL]
- 10.13 MCP Elicitation and OAuth [Codex]
...
- **Source**: common-workflows.md, https://developers.openai.com/codex/concepts/customization | Tier 1
- **Recommendation**: Git integration patterns for generated environments:
  - Document safe inspection operations: `git diff`, `git status`, `git log`
  - Require confirmation for: `git push`, `git reset --hard`
  - Treat force-push as explicitly requested only
...
- **Anti-pattern**: Not including VCS permissions in generated .codex/config.toml. Users get
  constant permission prompts for routine git operations, causing approval fatigue.

## 10.2 Build Systems [ALL]

- **Established**: Baseline
- **Source**: https://developers.openai.com/codex/concepts/customization, common-workflows.md | Tier 1
- **Recommendation**: Include build commands in AGENTS.md and GETTING_STARTED.md:
  - `npm run build`, `npm run test`, `npm run lint` for Node projects
  - `cargo build`, `cargo test` for Rust projects
  ...
  language. Codex performs dramatically better with self-verification.
- **Anti-pattern**: Not providing verification commands. Without them, you become the only
  feedback loop, and Codex cannot self-correct.

## 10.3 External Tools and CLIs [ALL]

- **Established**: Baseline
- **Source**: https://developers.openai.com/codex/concepts/customization | Tier 1
- **Recommendation**: Document available CLI tools for Codex: `gh`, `aws`, `gcloud`,
  `sentry-cli`, `terraform`, `kubectl`. Codex can learn unfamiliar CLIs by using their
  `--help` output. Include in AGENTS.md: "Use 'foo-cli-tool --help' to learn about foo
  tool, then use it to solve A, B, C."

  MCP servers connect external services: Notion, Figma, databases, monitoring, issue
  trackers. Use `codex mcp add` to configure. See sections 10.6-10.11 for full MCP
  configuration guidance.
- **Anti-pattern**: Expecting Codex to discover tools on its own. Explicitly list available
  CLI tools and their primary use cases in AGENTS.md.

## 10.4 Settings File Separation [ALL]

- **Established**: 2026-02
- **Source**: production game project production environment | Tier 2
- **Recommendation**: Use `local config profile` for machine-specific configuration
  (engine paths, local tool paths, user-specific hooks) and `.codex/config.toml` for
  shared team configuration (permissions, deny rules, ecosystem settings).

  `local config profile` is gitignored and contains paths that vary between developer
  machines. This prevents merge conflicts on team projects and avoids hardcoding
  local paths in shared configuration.

  Example split:
  - `.codex/config.toml`: permission rules, deny rules, ecosystem profiles, hook definitions
  - `local config profile`: absolute paths to engines/tools, user-specific Bash permissions,
    local MCP server configurations
- **Anti-pattern**: Putting machine-specific paths in .codex/config.toml. On team projects,
  every developer has different paths, causing constant merge conflicts or broken configs.

## 10.5 CI/CD Integration [ALL]

- **Established**: 2026-02
- **Source**: https://developers.openai.com/codex, GitHub Actions documentation | Tier 1
- **Recommendation**: For software development environments using GitHub, suggest
  integrating Codex GitHub Actions for automated PR review and issue triage.
  Setup: `/install-github-app` in Codex CLI.
...
- **Recommendation**: Three transport options for MCP servers:
  - **HTTP** (recommended): `codex mcp add --transport http <name> <url>` -- modern,
    efficient, recommended for new integrations
  - **SSE** (deprecated): `codex mcp add --transport sse <name> <url>` -- legacy,
    streaming server-sent events
  - **stdio** (local): `codex mcp add --transport stdio <name> -- <command> [args]` --
    for local processes

  Windows note for stdio with npx: use `cmd /c` wrapper:
  `codex mcp add --transport stdio name -- cmd /c npx -y @package`
...
- **Recommendation**: MCP server scopes control visibility:
  - **local** (default): Private, current project only. Stored in `~/.codex.json`
  - **project**: Shared via `.mcp.json` (checked into VCS). Visible to all team members.
  - **user**: Available across all projects. Stored in `~/.codex.json`
...
- **Recommendation**: Always use fully qualified names for MCP tools:
  ...
  In permission rules: `mcp__server__tool`. Without the server prefix, Codex may fail to
...
- **Recommendation**: Every MCP server placed in a generated `.mcp.json` or .codex/config.toml
...
- **Recommendation**: When MCP tool definitions exceed 10% of the context window, MCP
  ...
  them and defers loading, using an on-demand search tool that loads only the tools Codex
...
- **Recommendation**: Three browser automation approaches exist for Codex,
  ...
  | **Playwright MCP** | ~114K/session (33+ tools, full accessibility tree inline) | No | Yes | Sandboxed agents without filesystem access (Codex Desktop, chat interfaces) |
  | **Chrome DevTools MCP** | ~9% context for tool defs | No | Yes | Performance profiling, Core Web Vitals, network/console debugging |
  | **Codex browser/app workflows** | ~7.7% context for tool defs | Yes (logged-in sessions) | No | Authenticated workflows (Gmail, Notion, etc.), multi-tab |

  **For Codex environments (shell access), Playwright CLI is the default
  ...
  **When to recommend each during generation**:
  ...
  - Need authenticated browser sessions -> Codex browser/app workflows
  ...
  **For generated environments**: When intake reveals browser automation needs,
  ...
  trade-off. Do NOT default to Playwright MCP for Codex environments --
  the 4x token overhead is unnecessary when shell access is available.
- **Anti-pattern**: Defaulting to Playwright MCP for Codex projects. MCP
...
- **Source**: developers.openai.com/codex v2.1.76-v2.1.97 | Tier 1
- **Recommendation**: April 2026 brought several MCP enhancements relevant to environment
  ...
  **Elicitation (v2.1.76)**: MCP servers can request structured input from the user
  mid-task via interactive dialog. Codex surfaces `Elicitation` and
  ...
  **Result size override (v2.1.91)**: MCP results can override result persistence size
  via `_meta["openai/maxResultSizeChars"]` up to 500k chars. Useful for servers that
