...
2. Ask Codex to map prompts, tools, evals, and retrieval sources.
3. Ask for one eval case or prompt-contract improvement.
4. Ask the reviewer to inspect failure modes and privacy risks.
...
Use this loop for the first real task so the harness produces evidence, not just
a successful setup check.
