...
class CodexHarnessCliTests(unittest.TestCase):
    ...
    def test_prepare_pilot_delegates_to_pilot_preparer(self):
        command, _ = self.run_cli(
            [
                ...
                "/tmp/pilot-records",
                "--pilot-record-out",
                "/tmp/pilot-records/external-rag-pilot.json",
                "--pilot-status",
                "prepared",
                "--pilot-notes",
                "prepared for reporter",
                "--force",
                "--json",
            ]
        )
    ...
    def test_prepare_pilot_batch_delegates_to_batch_preparer(self):
        command, _ = self.run_cli(
            [
                "prepare-pilot-batch",
                "--record-dir",
    ...
    def test_pilot_handoff_delegates_to_handoff_exporter(self):
        command, _ = self.run_cli(
            [
                "pilot-handoff",
                "--record-dir",
    ...
    def test_beta_status_delegates_to_status_dashboard(self):
        command, _ = self.run_cli(
            [
                "beta-status",
                "--record-dir",
