...
class ProofNextTests(unittest.TestCase):
    ...
    def test_build_payload_turns_usage_gaps_into_operator_commands(self):
        ...
        self.assertTrue(any("codex-harness doctor --beta-exit" in command for command in commands))
        self.assertTrue(any("codex-harness proof-status --beta-exit" in command for command in commands))
        doctor_index = next(index for index, command in enumerate(commands) if "codex-harness doctor --beta-exit" in command)
        proof_index = next(index for index, command in enumerate(commands) if "codex-harness proof-status --beta-exit" in command)
        self.assertLess(doctor_index, proof_index)
    ...
    def test_write_report_includes_next_pilot_and_claim_boundary(self):
        ...
        self.assertIn("run beta-exit doctor", text)
