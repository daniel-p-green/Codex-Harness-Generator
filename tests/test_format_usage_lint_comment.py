...
class FormatUsageLintCommentTests(unittest.TestCase):
    def test_formats_conversion_ready_comment_without_claiming_proof(self):
        ...
        self.assertIn("--include-comments", comment)
        self.assertIn("--record-dir Docs/Environment/usage-records", comment)
        self.assertIn("--pilot-record-dir Docs/Environment/pilot-records", comment)
        self.assertIn("--no-write --json", comment)
