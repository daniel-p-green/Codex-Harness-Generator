...
class PilotHandoffTests(unittest.TestCase):
    ...
    def test_build_payload_and_write_handoff_folder(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue((handoff / "RETURN_PACKET.md").exists())
            ...
            self.assertIn("RETURN_PACKET.md", reporter_handoff)
            return_packet = (handoff / "RETURN_PACKET.md").read_text(encoding="utf-8")
            self.assertIn("NEXT_TASK.md", return_packet)
            self.assertIn("USAGE_REPORT_DRAFT.md", return_packet)
            self.assertIn("usage-from-issue", return_packet)
            self.assertIn("not usage proof", return_packet)
