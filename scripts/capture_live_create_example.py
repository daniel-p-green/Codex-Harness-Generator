...
def build_prompt(target: Path, project_brief: str, project_type: str, notes: str, source_label: str) -> str:
    trigger_command = " ".join(
        [
            "python",
            "scripts/simulate_create_trigger.py",
            shell_quote(target.as_posix()),
            "--project-type",
...
def run_codex_create(
    ...
    command.append(build_prompt(target, project_brief, project_type, notes, source_label))
...
def remove_excluded_references(destination: Path) -> None:
    for relative in [
        Path("Docs") / "Environment" / "MANIFEST.md",
        Path("Docs") / "Environment" / "ARCHITECTURE.md",
    ]:
...
def capture_live_example(
    ...
    remove_excluded_references(destination)
...
def main() -> int:
    ...
    parser.add_argument("--project-type", default="not specified", help="Stated project type written by the live trigger handoff")
    parser.add_argument("--notes", default="live-create capture", help="Additional trigger notes for --run-codex")
